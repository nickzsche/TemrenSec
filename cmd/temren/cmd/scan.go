...
func init() {
	...
	scanCmd.Flags().StringVar(&authUser, "auth-user", "", "Username for basic auth or form login")
	scanCmd.Flags().StringVar(&authPass, "auth-pass", "", "Password for basic auth or form login")
	scanCmd.Flags().StringVar(&authLoginURL, "auth-login-url", "", "Form-login URL; POST credentials here and reuse the session cookies")
	scanCmd.Flags().StringVar(&authUserField, "auth-user-field", "username", "Form field name for the username")
	scanCmd.Flags().StringVar(&authPassField, "auth-pass-field", "password", "Form field name for the password")
	scanCmd.Flags().StringArrayVar(&authLoginFields, "auth-login-field", nil, "Extra login form field (key=value, repeatable, e.g. CSRF token)")
...
func runScan(cmd *cobra.Command, args []string) {
	...
	// Form-based login: authenticate once, reuse the session cookies for the
	// whole scan so protected pages are reachable.
	if authLoginURL != "" {
		fields := map[string]string{
			authUserField: authUser,
			authPassField: authPass,
		}
		for _, f := range authLoginFields {
			parts := strings.SplitN(f, "=", 2)
			if len(parts) == 2 {
				fields[parts[0]] = parts[1]
			}
		}
		if err := client.FormLogin(ctx, authLoginURL, fields); err != nil {
			fmt.Fprintf(os.Stderr, "[!] Form login failed: %v\n", err)
			os.Exit(1)
		}
		if !silent {
			fmt.Println("[*] Form login successful; scanning as authenticated user")
		}
	}
