...
// clientIDCounter guarantees uniqueness even when two IDs are generated within
// the same clock tick (Windows timers have coarse resolution, so UnixNano can
// repeat back-to-back).
...
func generateClientID() string {
	...
	var r [4]byte
	_, _ = rand.Read(r[:])
	return fmt.Sprintf("client_%d_%d_%s", time.Now().UnixNano(), n, hex.EncodeToString(r[:]))
}
