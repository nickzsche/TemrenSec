...
func NewScanEngine(client *httpengine.Client, scanners []Scanner, concurrency int) *ScanEngine {
	return &ScanEngine{
		...
		verify:         true,             // deterministic proof layer on by default
	}
}

// SetVerify toggles the deterministic proof layer that re-tests every
// Critical/High finding after a scan (see ProofVerifier.ApplyVerification).
// Enabled by default; disable it for the fastest possible scan.
func (e *ScanEngine) SetVerify(v bool) { e.verify = v }

// finalize applies OWASP 2025 mapping, dedup and (optionally) proof
// verification. It is the single funnel both scan paths return through.
func (e *ScanEngine) finalize(ctx context.Context, findings []Finding) []Finding {
	fillOWASP2025(findings)
	findings = DedupFindings(findings)
	if e.verify {
		findings = NewProofVerifier(e.client).ApplyVerification(ctx, findings)
	}
	return findings
}
...
func (e *ScanEngine) RunAll(ctx context.Context, targets []string) ([]Finding, error) {
	...
	wg.Wait()
	return e.finalize(ctx, allFindings), nil
}
